...
/*ADD*/		case CONVEX_HULL_SHAPE_PROXYTYPE:
/*ADD*/			{
/*ADD*/				// draw the convex hull shape...whatever it is
/*ADD*/				DrawConvexHull(pShape);
/*ADD*/				break;
/*ADD*/			}
...
/*ADD*/		// get the polyhedral data from the convex hull
/*ADD*/		const btConvexPolyhedron* pPoly = shape->isPolyhedral() ? ((btPolyhedralConvexShape*) shape)->getConvexPolyhedron() : 0;
/*ADD*/		if (!pPoly) return;
/*ADD*/	
/*ADD*/		// begin drawing triangles
/*ADD*/		glBegin (GL_TRIANGLES);
/*ADD*/	
/*ADD*/		// iterate through all faces
/*ADD*/		for (int i = 0; i < pPoly->m_faces.size(); i++) {
/*ADD*/			// get the indices for the face
/*ADD*/			int numVerts = pPoly->m_faces[i].m_indices.size();
/*ADD*/			if (numVerts>2)	{
/*ADD*/				// iterate through all index triplets
/*ADD*/				for (int v = 0; v <pPoly->m_faces[i].m_indices.size()-2;v++) {
/*ADD*/					// grab the three vertices
/*ADD*/					btVector3 v1 = pPoly->m_vertices[pPoly->m_faces[i].m_indices[0]];
/*ADD*/					btVector3 v2 = pPoly->m_vertices[pPoly->m_faces[i].m_indices[v+1]];
/*ADD*/					btVector3 v3 = pPoly->m_vertices[pPoly->m_faces[i].m_indices[v+2]];
/*ADD*/					// calculate the normal
/*ADD*/					btVector3 normal = (v3-v1).cross(v2-v1);
/*ADD*/					normal.normalize ();
/*ADD*/					// draw the triangle
/*ADD*/					glNormal3f(normal.getX(),normal.getY(),normal.getZ());
/*ADD*/					glVertex3f (v1.x(), v1.y(), v1.z());
/*ADD*/					glVertex3f (v2.x(), v2.y(), v2.z());
/*ADD*/					glVertex3f (v3.x(), v3.y(), v3.z());
/*ADD*/				}
/*ADD*/			}
/*ADD*/		}
/*ADD*/		// done drawing
